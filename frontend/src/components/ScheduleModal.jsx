...
export default function ScheduleModal({ defaultType = 'recycling', onClose, onAdded }) {
  ...
  async function handleAdd() {
    ...
    try {
      ...
      if (token) {
        const res = await fetch((import.meta.env.VITE_API_URL || 'http://127.0.0.1:3000') + '/api/schedules', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
          body: JSON.stringify({ type, scheduled_at: d.toISOString(), time_label: time, location: location ? { lat: location.lat, lng: location.lng } : null })
        });
    ...
    <div className="fixed inset-0 z-50 grid place-items-center">
      ...
      <div className="relative bg-white rounded-lg p-6 w-full max-w-2xl z-10">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <div className="text-gray-600 mb-2">Click map to pick pickup location</div>
            <div ref={mapEl} className="h-64 rounded overflow-hidden" />
            <div className="text-xs text-gray-500 mt-2">Selected: {location ? `${location.lat.toFixed(5)}, ${location.lng.toFixed(5)}` : 'None'}</div>
          </div>
        </div>
      </div>
    </div>
