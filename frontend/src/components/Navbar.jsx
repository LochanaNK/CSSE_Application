      ...
      <nav className="bg-white shadow-md px-8 py-4 flex justify-between items-center ">
        ...
        <div className="flex items-center gap-3 relative" ref={dropdownRef}>
          <button onClick={() => setOpen(v => !v)} className="flex items-center gap-3 focus:outline-none">
            <span className="text-gray-500 text-sm">Hi, {user?.name ? user.name.split(' ')[0] : (user?.email || 'Guest')}</span>
            <img src={user?.email ? `https://i.pravatar.cc/40?u=${user.email}` : 'https://i.pravatar.cc/40'} alt="user" className="rounded-full w-10 h-10 border border-gray-300" />
          </button>

          {open && (
            <div className="absolute right-0 mt-12 w-44 bg-white rounded shadow-lg py-2 z-40">
              <button onClick={() => { setShowProfile(true); setOpen(false); }} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100">Profile</button>
